...
const deleteRole = async (req, res) => {	
    try {
        /* ============================ start: validatoin ================================= */ 
            ...
            if(exist.length == 0) return fail(res,400,null,"Role does not exist") ;
        /* ================================ End: validatoin ================================= */ 
...
const createRole = async (req, res) => {
    try {
        /* ================== start: validatoin =================== */ 
            ...
            if(exist.length > 0) return fail(res,400,null,"Role exist");
        /* ================== start: validatoin =================== */ 
...
const updateRole = async (req, res) => {
    try {
        ...
        /* ======= Start:: Update roles =================== */ 
            roles.update( { rolename },{where : {id}}).then(role => {
                return success(res,200,role,"Updated");
            })
        /* ========= End:: Update roles ================== */         
    } catch (error) {
        return sendError(res,500,null,error.message);
...
const assignPermssion = async(req,res) => {
    try {
        ...
        if(pExist.length == 0) return fail(res,400,null,"Permission does not exist") ;;


        /* =================== Start:: Validate if role exist ==================  */ 
        const rExist = await roleExist(roleid);
        if(rExist.length == 0 ) return fail(res,400,null,"Role does not exist");
...
const removePermission = async (req,res) => {
    try {
        ...
        if(!roleHasPermssion) return fail(res,400,null,"Role does not have that permission");
...
const assignRole = async (req,res) => {
    try {
        ...
        /* =============== Start:: validate if role exist ==================  */ 
            ...
            if(!exist) return fail(res,400,null,"Role does not exist");

        /* =============== Start:: validate if role exist ==================  */
            exist =  user[0].roleId ==  roleId;
            if(exist) return fail(res,400,null,"Role already assigned");    
...
const removeRole = async (req,res) => {
    try {
        ...
        /* =============== Start:: validate if role exist ==================  */
            exist =  user[0].roleId ==  roleId;
            if(!exist) return fail(res,400,null,"This user does not have this role");    
